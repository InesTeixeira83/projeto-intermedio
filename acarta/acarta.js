...
async function inicializar() {
    ...
    scenes.casa2.image.src = scenes.casa.sprite
    await scenes.casa2.image.decode()
    scenes.casa2.image = new Image()
    scenes.casa2.image.src = scenes.casa2.sprite
    await scenes.casa2.image.decode()
    // init actors
    actors.supermario.image = new Image()
    actors.supermario.image.src = actors.mario.sprite
    await actors.supermario.image.decode()
    // init stage
    stage.scene = scenes.casa2
    stage.actors.push(actors.supermario)
    
    actors.supermario.scenePos = bound.width / 2 - actors.supermario.image.width / 2  // initial position middle of scene
...
async function actorTo(actor, x) {
    ...
    actor.x = x
    // update scene
    const y = floor - actors.supermario.image.height
